...
To prepare the data for visualization, I decided to choose the most recent year in the table that had values for every country available (2008). Then, I copied these values and created a new Excel spreadsheet with two rows: one for the country name and the other for its corresponding value. Below is a screenshot of the original table on the left and a screenshot of the <a href="/HW3/Resources/r&d.xlsx">new table</a> on the right (what I used to create the bar chart).
